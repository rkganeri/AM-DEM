...
TEST(particles,calcWallForce) {
    ...
    for (int i=0; i<3; i++) {
        print(fmt::format("particle {}", i));
        print(fmt::format(" psi_con: ({:14.7e},{:14.7e},{:14.7e})", h_psi_con(i,0),
              h_psi_con(i,1), h_psi_con(i,2)));

        print(fmt::format(" psi_fric: ({:14.7e},{:14.7e},{:14.7e})", h_psi_fric(i,0),
              h_psi_fric(i,1), h_psi_fric(i,2)));
    }
    ...
    //EXPECT_FLOAT_EQ(h_psi_con(1,1), 0);
    //EXPECT_FLOAT_EQ(h_psi_con(1,2), 0);
