...
// tests particle-wall force calculations
TEST(particles,calcWallForce) {
    ...
    EXPECT_FLOAT_EQ(h_mass(1), 4/3.*M_PI*pow(h_radius(1),3)*7952); // 1.5414290e-10
...
// tests full particle force calculations
TEST(particles,calcForces) {
    ...
    // get view back on host so we can query them
    Kokkos::View<double**>::HostMirror h_psi_tot = Kokkos::create_mirror_view(psi_tot);
