...
Open(unit=100, file='PowderDepositionKhairallah600.dat')
...
Open(unit=200, file='PowderDepositionKhairallahFinal600.dat')

! Generate a set of particles and randomly place them in a box
length = 0.0010D0 ! [m]
width = 0.00050D0
height = 0.00030D0

meanradius = 13.5e-6 !average particle diamter of 50 microns, 95% of particles from 30 - 70 microns in diameter
stddevradius = 4.0e-6
minradius = 4.0e-6
maxradius = 23.0e-6
numlayers = 1
...
do layer = 1,1
   ...
   do i = 1,numparticles 
      ...
      CenterCoords(i,3) = Height - Radius(i) - 0.333*height*RandNumbers(i,3)
   end do
   ...
   do while (i > 0)
      ...
      do while (N <= numparticles)
         ...
         if (distance<(Radius(i)+Radius(N))) then
            ...
            CenterCoords(i,3) = Height -Radius(i) - 0.333*height*RandNumVec(3)
            N = i+1
         else
            N = N+1
         end if
...
!!!OUTPUT FILES TO TECPLOT every 25000 time steps (100 total)
      if ((counter==25000*k) .OR. (counter==1)) then
...
!!!!PARTICLE TO PARTICLE COLLISIONS - WITH BINNING
  do i = 1,numparticles
