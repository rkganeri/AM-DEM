...
Bins::setBins(const std::unique_ptr<Particles>& particles) {
    ...
    const double bin_hieght = gs.height_ / num_bins_z_;
    double distance = 0.;
    ...
    Kokkos::parallel_for("set_particle_bins", mdrange_policy2({0,0},{num_particles_,3}), 
        ...
        particle_bin_(i,j) = (int) particles_coordsnp1_(i,j)/distance;
    });
    ...
    Kokkos::View<double**>::HostMirror h_particle_bin = Kokkos::create_mirror_view(particle_bin_);
    Kokkos::View<double***>::HostMirror h_bins = Kokkos::create_mirror_view(bins_);
    Kokkos::View<double*>::HostMirror h_linked_list = Kokkos::create_mirror_view(linked_list_);
